...
class PrioritizedReplayMemory:
    ...
    def __init__(self, capacity):
        ...
        self.current_size = 0  # 添加当前有效经验计数器
        self.is_full=False
    ...
    def random_add(self, error, sample):
        ...
        """
        p = self.get_priority(error)
        self.sumtree.random_add(p, sample)
    ...
    def get_importance_sampling_weight(self, p):
        ...
        #p = max(p, 1e-12)
        if p == 0:
            print("[ERROR]get_importance_sampling_weight_ p is 0 ")
        

        return (p * self.capacity) ** (-self.b)  # equivalent to ((1./self.capacity) * (1./p)) ** self.b

    def size(self):
        return self.sumtree.total()
    ...
    def sample(self, n):
        ...
        batch = []
        segment = self.sumtree.total() / n
        
        self.b = min(1, self.b + self.b_increment)
        start=self.sumtree.capacity-1

        if self.current_size >=self.capacity:
            min_prob = np.min(self.sumtree.tree[-self.sumtree.capacity:]) / self.sumtree.total()
        else:
            min_prob = np.min(self.sumtree.tree[start:start+self.current_size]) / self.sumtree.total()

        
        if min_prob == 0:
            print("[ERROR]sample min_prob is 0 ")
        ...
        for i in range(n):
            ...
            #print(f"idx[{idx}]_data[{data}]_p[{p}]")
            sampling_prob = p / self.sumtree.total()
